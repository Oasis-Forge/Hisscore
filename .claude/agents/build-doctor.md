...
1. If the caller gave a log path, use it. Otherwise run the build they named with output redirected to a file, e.g. `cd Hisscore; flutter build apk --release *> "$env:TEMP\build-doctor.log"`. The stack's SDK paths are in `docs/STACK_NOTES.md`.
