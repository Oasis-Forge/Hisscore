...
1. `cd Hisscore; dart format --output=none --set-exit-if-changed .`
2. `cd Hisscore; flutter analyze`
3. `cd Hisscore; flutter test -r failures-only`
...
Don't paste raw logs. Don't fix anything unless the same request asked for fixes (a format failure is fixed by `cd Hisscore; dart format .`).
